<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsPrices.aspx.cs" Inherits="CdHotelManage.Web.Admin.Menus.GoodsPrices" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function openupadd(obj, ids, type) {
            var title = "";
            var url = "GoodsPriceAdds.aspx?id=" + ids + "&type=" + type;
            if (type == 0) {
                title = "添加商品费用";
            } else {
                title = "编辑商品费用";
            }
            showMyWindow(title, url, 280, 350, true, true, true);
        }
        ...
        function deletes(ids) {
            ...
            document.getElementById("txt_id").value = ids;
            if (confirm("确定要删除吗？")) {
                $.get("/admin/ajax/IsDel.ashx", "type=goodsCate&id=" + ids, function (obj) {
                    if (obj == "ok") {
        ...
        function ShowDiv(cid, obj) {
            ...
            $.get("/Admin/Ajax/Books.ashx?r=" + Math.random(), "type=getGoods&cid=" + cid, function (obj) {
                switch (obj.statu) {
                    ...
                    case "ok":
                        ...
                        for (var i = 0; i < tbList.length; i++) {
                            ts += "<tr class=\"tr1 trtop\"><td>" + tbList[i].Goods_number + "</td><td>" + tbList[i].Goods_name + "</td><td>" + GetName(tbList[i].Goods_categories) + "</td><td>" + tbList[i].Goods_unit + "</td><td>" + tbList[i].Goods_price + "</td><td>" + tbList[i].Goods_jf + "</td><td><span class=\"fontblue\"><a href=\"#\" class=\"aedit\" onclick=\"openupadd(this," + tbList[i].id + ",1)\">[编缉]</a> <a href=\"#\" onclick=\"BookEancel(" + tbList[i].id + ")\">[删除]</a></span></td></tr>";
                        }
                        $("#Tabcox .tr").after(ts);
                        break;
                    case "err":
        ...
        function GetName(id) {
            var txt = "";
            $.get("/Admin/Ajax/Books.ashx?r=" + Math.random(), "type=getNames&id=" + id, function (obj) {
                txt = obj;
            }, "text");
            return txt;
        }
...
<body onload="clo()">
    ...
    <div class="fyszbt">
        <p>
            <span class="span01 color000">商品设置 </span><span class="color000">商品名称和费用名称不能一样。</span></p>
        <input type="text" id="txt_NumberName" runat="server" class="fyIpt fyIpt1" /><asp:Button
            CssClass="lanbtn" ID="button02" runat="server" Text="查询" OnClick="button02_Click" />
    </div>
    <div class="conTree" style="">
        ...
        <div class="rightTable" style=" width:80%;">
            ...
            <div class="cart-page">
                <webdiyer:aspnetpager id="Pager" cssclass="sbsjhand" runat="server" firstpagetext="首页"
                    lastpagetext="尾页" nextpagetext="下一页" alwaysshow="True" prevpagetext="上一页" nextprevbuttonclass="incoleft"
