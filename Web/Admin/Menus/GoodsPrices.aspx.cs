...
{
    public partial class GoodsPrices : System.Web.UI.Page
    {
        ...
        {
            txt_name.Value = "";
            string Content = "<ul><li onclick=\"ShowDiv('0')\" style=\"background:#4486b7; height:30px; color:#fff;\">&nbsp;&nbsp;&nbsp;全部</li>";
            DataSet dt = fmcost.GetList(" Goods_ifType=0");
            foreach (DataRow dr in dt.Tables[0].Rows)
            {
                Content += "<li class=\"lis\" onclick=\"ShowDiv('" + dr["id"] + "',this)\">&nbsp;&nbsp;&nbsp;" + dr["Goods_name"] + "<a href='#' class=\"dela\" onclick='deletes(" + dr["id"] + ")'><img src='../../images/scicon.png' width='14' height='16' /></a><a class=\"modif\" onclick=\"Goodsupa(this," + dr["id"] + ",0)\" href='#'><img src='../../images/bj_icon.png' width='14' height='16' /></a></li>";
            }
            Content += "</ul>";
        ...
        public void Shwere()
        {
            ...
            stwhere = " WHERE 1=1 and Goods_ifType=1 ";
            if (txt_NumberName.Value.Length > 0)
            {

                stwhere += " and (Goods_number like '%" + txt_NumberName.Value + "%' or Goods_name like '%" + txt_NumberName.Value + "%')";
            }
        ...
        #region 获得列表
        private void GetListRoom(IList<Model.Goods> list)
        {

            foreach (Model.Goods room in list)
            {
                sbHtml.Append("<tr class=\"tr1 trtop\">");
                sbHtml.Append("<td>" + room.Goods_number + "</td>");
                sbHtml.Append("<td>" + room.Goods_name + "</td>");
                sbHtml.Append("<td>" + Getname(Convert.ToInt32(room.Goods_categories)) + "</td>");
                sbHtml.Append("<td>" + room.Goods_unit + "</td>");
                sbHtml.Append("<td>" + Convert.ToDecimal(room.Goods_price).ToString("0.##") + "</td>");
                sbHtml.Append("<td>" + Convert.ToDecimal(room.Goods_jf).ToString("0.##") + "</td>");
