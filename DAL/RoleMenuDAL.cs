...
namespace CdHotelManage.DAL
{
	...
	public partial class RoleMenuDAL
	{
		...
		public bool Add(CdHotelManage.Model.RoleMenu model)
		{
			...
			strSql.Append("RoleID,Menu_id,Menu_pid)");
			...
			parameters[2].Value = model.Menu_pid;
		...
		public CdHotelManage.Model.RoleMenu GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 RoleID,Menu_id,Menu_pid from RoleMenu ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};

			CdHotelManage.Model.RoleMenu model=new CdHotelManage.Model.RoleMenu();
		...
		/// 得到一个对象实体
		/// </summary>
		public CdHotelManage.Model.RoleMenu DataRowToModel(DataRow row)
		{
			CdHotelManage.Model.RoleMenu model=new CdHotelManage.Model.RoleMenu();
			if (row != null)
			{
				...
				if(row["Menu_pid"]!=null && row["Menu_pid"].ToString()!="")
				{
					model.Menu_pid=int.Parse(row["Menu_pid"].ToString());
				}
			}
			return model;
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select RoleID,Menu_id,Menu_pid ");
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from RoleMenu T ");
        ...
        {
            string sql = "delete from RoleMenu where RoleID=" + roleid;
